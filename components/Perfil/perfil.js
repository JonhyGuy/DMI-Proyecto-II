import React, { useEffect, useState } from "react";
import { View, Text, Image, TouchableOpacity } from "react-native";
import CameraComponent from "./camera";
import { useIsFocused } from "@react-navigation/native";
import { Camera } from "expo-camera";
import * as ImagePicker from "expo-image-picker";

import { storage } from "../../firebase";

export default function PerfilScreen() {
    const [startCamera, setStartCamera] = useState(false);
    const [previewVisible, setPreviewVisible] = useState(true);
    const [capturedImage, setCapturedImage] = useState(null);

    const [guardar, setGuardar] = useState(false);
    const isFocused = useIsFocused();

    const [photoDefault, setPhotoDefault] = useState("data:image/png;base64,")
    const [photo, setPhoto] = useState("");

    const __startCamera = async () => {
        const { status } = await Camera.requestCameraPermissionsAsync();
        if (status === "granted") {
            setPreviewVisible(false);
            setStartCamera(true);
            setCapturedImage(null);
        } else {
            Alert.alert("Access denied");
        }
    };

    useEffect(() => {
        __getProfilePhoto();
    }, []);

    useEffect(() => {
        if (capturedImage != null) {
            setPhoto(capturedImage.uri);
            setGuardar(true);
        } else {
            setGuardar(false);
        }
    }, [capturedImage]);

    const __getProfilePhoto = async () => {
        await storage.ref("usuario1/profile").getDownloadURL().then(profile => {
            setPhoto(profile);
            setCapturedImage(null);
        }).catch(e => {
            setPhoto(photoDefault);
            setCapturedImage(null);
        });
    }

    const __back = () => {
        setStartCamera(false);
        setPreviewVisible(true);
        setGuardar(false)
        __getProfilePhoto();
    }

    const _guardar = async () => {
        const response = await fetch(capturedImage.uri)
        const blob = await response.blob();
        await storage.ref("usuario1/profile").put(blob).then(res => {
            __getProfilePhoto();
        }).catch(err => {
            __getProfilePhoto();
        });
    }

    const __cancel = () => {
        __back();
    }

    const pickImage = async () => {
        let result = await ImagePicker.launchImageLibraryAsync({
            mediaTypes: ImagePicker.MediaTypeOptions.All,
            allowsEditing: true,
            aspect: [4, 3],
            quality: 1,
        });

        if (!result.cancelled) {
            setCapturedImage(result);
        }
    };

    return (
        <View style={{ width: '100%', flex: 1 }}>
            {(isFocused) ?

                (previewVisible) ?

                    (<View style={{ width: '100%', flex: 1 }}>
                        <View style={{ alignItems: "center" }}>
                            <Image source={{ uri: photo }} style={{ width: 300, height: 300, borderRadius: 100 }} />
                        </View>
                        {
                            (!guardar) ?
                                <View style={{
                                    display: "flex",
                                    width: "100%",
                                    alignItems: "center",
                                    alignContent: "center",
                                }}>
                                    <View style={{
                                        flexDirection: "row"
                                    }}>
                                        <TouchableOpacity onPress={__startCamera} style={{ width: 150, height: 50, backgroundColor: "black", margin: 10, justifyContent: 'center' }}>
                                            <Text style={{ color: "white", fontSize: 20, textAlign: "center" }}>Tomar Foto</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={{ width: 150, height: 50, backgroundColor: "black", margin: 10, justifyContent: 'center' }} onPress={pickImage}>
                                            <Text style={{ color: "white", fontSize: 20, textAlign: "center" }}>Ir a la galeria</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                :
                                <View style={{
                                    display: "flex",
                                    width: "100%",
                                    alignItems: "center",
                                    alignContent: "center",
                                }}>
                                    <View style={{
                                        flexDirection: "row"
                                    }}>
                                        <TouchableOpacity style={{ width: 150, height: 50, backgroundColor: "black", margin: 10, justifyContent: 'center' }}>
                                            <Text style={{ color: "white", fontSize: 20, textAlign: "center" }} onPress={_guardar}>Guardar</Text>
                                        </TouchableOpacity>

                                        <TouchableOpacity style={{ width: 150, height: 50, backgroundColor: "black", margin: 10, justifyContent: 'center' }}>
                                            <Text style={{ color: "white", fontSize: 20, textAlign: "center" }} onPress={__cancel}>Cancelar</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                        }
                    </View>)
                    :
                    (<View style={{ width: "100%", height: "100%" }}>
                        <TouchableOpacity onPress={__back} style={{ width: 100, height: 40, backgroundColor: "red", margin: 10, justifyContent: 'center' }}>
                            <Text style={{ color: "white", fontSize: 20, textAlign: "center" }}>Regresar</Text>
                        </TouchableOpacity>
                        < CameraComponent
                            setCapturedImage={setCapturedImage}
                            setPreviewVisible={setPreviewVisible}
                            setStartCamera={setStartCamera}
                        />
                    </View>)
                :
                <Text></Text>
            }
        </View >
    );
}